...
namespace coord_trans_api
{
	...
	struct MatrixOperation
	{
		...
		void zx_to_dx_matrix(double L, double B, double H, double* m)
		{
			...
			R1[0] = cos(M_PI/2-newB);  R1[1] = 0; R1[2] = sin(M_PI/2-newB);
			R1[3] = 0;                 R1[4] = 1; R1[5] = 0;
			R1[6] = -R1[2];            R1[7] = 0; R1[8] = R1[0];
	
			R2[0] = cos(newL);  R2[1] = - sin(newL); R2[2] = 0;
			R2[3] = -R2[1];     R2[4] = R2[0];      R2[5] = 0;
    ...
    void global2local( double& x, double& y, double& z, \
            double refLon, double refLat, double refAlt )
    {
		MatrixOperation mo;
    ...
    void local2global( double& x, double& y, double& z, \
            double refLon, double refLat, double refAlt )
    {
		MatrixOperation mo;
    ...
    void lonlat2local( double& x, double& y, double& z, \
            double refLon, double refLat, double refAlt )
    {
        lonlat2global( x, y, z );
	...
	void local2lonlat( double& lon, double& lat, double& ele, \
			double refLon, double refLat, double refAlt )
	{
	...
	void global2enu( double& x, double& y, double& z, \
			double refLon, double refLat, double refAlt )
	{
		global2local( x, y, z, refLon, refLat, refAlt );
	...
	void enu2global( double& x, double& y, double& z, \
			double refLon, double refLat, double refAlt )
	{
		enu2seu( x, y, z, x, y, z );
	...
	void lonlat2enu( double& x, double& y, double& z, \
			double refLon, double refLat, double refAlt )
	{
	...
	void enu2lonlat( double& x, double& y, double& z, \
			double refLon, double refLat, double refAlt )
	{
		enu2seu( x, y, z, x, y, z );
	...
	void seu2enu( double& enuX, double& enuY, double& enuZ, \
			double seuX, double seuY, double seuZ )
	{
		enuX = seuY;
		enuY = -seuX;
		enuZ = seuZ;
	...
	void enu2seu( double& seuX, double& seuY, double& seuZ, \
			double enuX, double enuY, double enuZ )
	{
		seuX = -enuY;
		seuY = enuX;
		seuZ = enuZ;
